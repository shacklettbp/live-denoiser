...
data_dir = args.noisy
dst_dir = args.dst
files = get_files(data_dir, 'exr', int(args.num_imgs), one_idx=True)
ref_files = get_files(args.ref, 'exr', int(args.num_imgs), one_idx=True) if args.ref is not None else None
cropsize = 256
...
for idx in range(0, len(files) - args.num_temporal, args.num_temporal + 1):
    hdr = []
    normal = []
    albedo = []
    ref = []
